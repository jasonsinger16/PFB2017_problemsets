...
mean_len_stmt = "Mean FASTQ sequence length: {:.2f}"
print(mean_len_stmt.format(gotlenstats[0]))
stdev_len_stmt = "Standard deviation of FASTQ sequence length: {:.2f}"
print(stdev_len_stmt.format(gotlenstats[1]))

gotqualstats = calc_mean_stdev_qual(line_list)
mean_qual_stmt = "Mean base quality scores over all FASTQ sequences: {:.2f}"
print(mean_qual_stmt.format(gotqualstats[0]))
stdev_qual_stmt = "Standard deviation of base quality scores over all FASTQ sequences: {:.2f}"
print(stdev_qual_stmt.format(gotqualstats[1]))
